...
static void timerHandler( int sig, siginfo_t *si, void *uc )
{
	...
	/* End Section - 2 */
	raise(SIGUSR1);  // pid = current process id
}
...
void RemoveDepHandler(int sig) {
	/* TODO This Section - 3 */
	//cout << "in RemoveHandler" << endl;
	// Foreach step that has been completed since last run, remove it as a dependency
	/* End Section - 3 */
	// iterate completedsteps -> i or while loop
		// pop element
		// receipeObj-> removedepency(completedcursteps)
	while (completedSteps ->size() != 0){
		recipeSteps ->RemoveDependency(completedSteps->at(0));
		completedSteps->erase(completedSteps->begin());
	}
}
...
int main(int argc, char **argv)
{
	...
	if (sigaction(SIGRTMIN, &sa, NULL) == -1){
		perror("Sigaction error with sa");
		}
	...
	// iterate until all the steps are completed => completeCount != receipeObj -> Count()
		// receipeObj -> getreadysteps
		// iterate over the getreadysteps
			// set running status to true
			// make timer(step[i], step[i]->timer)
	...
	while (recipeSteps->Count() != completeCount){
		for(Step* i : recipeSteps ->GetReadySteps()) {
			i->running = true;
			makeTimer(i, i->duration);
		}
	}
